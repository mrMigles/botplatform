...
public class CatImageOverlay {
  ...
  private static void drawStringCentered(Graphics g, String text, BufferedImage image, boolean top)
    ...
    int height = 0;
    int fontSize = top ? MAX_FONT_SIZE - 4 : MAX_FONT_SIZE;
    int maxCaptionHeight = image.getHeight() / 5;
    int maxLineWidth =
        top ? image.getWidth() - SIDE_MARGIN * 2 + 50 : image.getWidth() - SIDE_MARGIN * 2 - 50;
    String formattedString = "";

    do {
      g.setFont(new Font("Arial", top ? Font.PLAIN : Font.BOLD, fontSize));
    ...
    int y = 0;
    if (top) {
      y = TOP_MARGIN + g.getFontMetrics().getHeight() - 10;
    } else {
    ...
    for (String line : formattedString.split("\n")) {
      ...
      if (top) {
        ...
        g.drawString(line, (image.getWidth() - (int) stringBounds.getWidth()) / 2 + 2 - 150, y + 2);
        g.setColor(Color.BLACK);
        g.drawString(line, (image.getWidth() - (int) stringBounds.getWidth()) / 2 - 150, y);
      } else {
        g.setColor(Color.WHITE);
        g.drawString(line, (image.getWidth() - (int) stringBounds.getWidth()) / 2 + 2 + 170, y + 2);
        g.setColor(Color.BLACK);
        g.drawString(line, (image.getWidth() - (int) stringBounds.getWidth()) / 2 + 170, y);
      }
