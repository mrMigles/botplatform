    ...
    public void process(TelegramMessageEntity messageEntity) throws TelegramApiException {
        ...
        if (StringUtils.isNotEmpty(mes) && mes.contains("/anon")) {
            ...
            if (messageEntity.getMessage().getSticker() != null) {
                ...
                messageEntity.getSender().sendSticker(new SendSticker().setSticker(messageEntity.getMessage().getSticker().getFileId()).setChatId(messageEntity.getChatId()));
            }
            else if (messageEntity.getMessage().getCaption() != null) {
                messageEntity.getSender().execute(new SendMessage().setChatId(messageEntity.getChatId()).setText("Кто-то прислал: \n"));
                messageEntity.getSender().sendPhoto(new SendPhoto().setCaption(messageEntity.getMessage().getCaption()).setChatId(messageEntity.getChatId()));
            }
