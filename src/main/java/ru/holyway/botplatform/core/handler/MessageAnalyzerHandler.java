...
public class MessageAnalyzerHandler implements MessageHandler {
  ...
  public String provideAnswer(MessageEntity messageEntity) {
    ...
    if (isJock(messageEntity)) {
      if (TimeUnit.MILLISECONDS.toSeconds(System.currentTimeMillis() - context.getLastStamp())
          < 10) {
  ...
  private String sendAnalize(MessageEntity message) {
    final String text =
        "Анализ использования:\nОтправлено сообщений: " + context.getCount() + "\nУдачных шуток: "
            + context.getGoodCount() + "\nШуток про Дениcа: " + context.getDenisCount()
            + "\nСамый поехавший: я";
    return text;
  }

  private boolean isJock(MessageEntity message) {
    if (message.getText() != null && (message.getText().contains("\uD83D\uDE04") || message
        .getText().contains("\uD83D\uDE03")
        || message.getText().contains("xD") || message.getText().contains(":D") || message.getText()
        .contains("хах")
        || message.getText().contains("лол") || message.getText().contains("lol") || message
        .getText().contains("Лол"))) {
      return true;
    }
    return false;
  }
